...
::: info
When an `Archived` event is appended to a stream (as shown in the example above), the stream is immediately marked as archived once the session is saved.

This means that asynchronous projections will not process any other events appended in the same session — even if those events were added before the `Archived` event.

If you rely on asynchronous projections and need all events to be processed before archiving, you can work around this by either:
